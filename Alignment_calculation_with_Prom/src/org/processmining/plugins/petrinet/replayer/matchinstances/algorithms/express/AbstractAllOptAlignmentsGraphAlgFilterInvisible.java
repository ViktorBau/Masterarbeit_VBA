 ...
 *
 */
public abstract class AbstractAllOptAlignmentsGraphAlgFilterInvisible<D extends AbstractPDelegate<T>, T extends Tail> extends AbstractAllOptAlignmentsAlg<D,T>{
	...
	protected int addReplayResults(D delegate, XTrace trace, MatchInstancesGraphRes r, TIntIntMap doneMap,
			...
			final List<Object> suffixNodeInstance, final List<StepTypes> suffixStepTypes, String filePath, String row) {
		AllSyncReplayResult repResult = recordToResult(delegate, trace, r.filteredTrace, r.records, traceIndex,
				...
				r.mapRecordToSameSuffix, filePath, row);
		if (mapRes == null) {
	...
	protected AllSyncReplayResult recordToResult(D d, XTrace trace, Trace filteredTrace, Collection<PRecord> records,
			...
			Map<Record, List<Record>> mapToStatesWSameSuffix, String filePath, String row) {
...
//		}
		...
		for (PRecord r : records) {
			...
			alignmentsBefore += allHistory.size();
			ArrayList<String> uniqueTraces = new ArrayList<String>();
			for (List<PRecord> history : allHistory) {
				...
				traceString = nodeInstance.toString() + stepTypes.toString();
				if (!uniqueTraces.contains(traceString)) {
		...
		System.out.println("#Alignments before filter" + alignmentsBefore);
		row += "," +  alignmentsBefore;
		System.out.println("#Alignments after filter" + lstNodeInstanceLst.size());
		row += "," +  lstNodeInstanceLst.size();
	...
	public static boolean containsList(List<Object> nodeInstance, List<List<Object>> lstNodeInstanceLst) {
		  for (List<Object> innerList : lstNodeInstanceLst) {
		    if (nodeInstance.equals(innerList)) {
		      return true;
		    }
		  }
		  return false;
