...
import org.processmining.plugins.petrinet.replayer.matchinstances.algorithms.express.AbstractAllOptAlignmentsGraphAlg;
import org.processmining.plugins.petrinet.replayer.matchinstances.algorithms.express.AllOptAlignmentsGraphThread;
...
@KeepInProMCache
public class AllOptAlignmentsGraphAlgDebug extends AbstractAllOptAlignmentsGraphAlgFilterInvisible<PDelegate, DijkstraTail> {
	...
	public PNMatchInstancesRepResult replayLog(final PluginContext context, PetrinetGraph net, Marking initMarking,
		...
		System.out.print("Finished calculation, now blwong up result");
		...
		Path pathToModel =(Path) parameters[3];
	
		Path filePath = pathToModel.resolve("Alignment_amount_stats.csv");
		String filePathString = filePath.toString();
		
		Path filePathLastTrace = pathToModel.resolve("last_trace.csv");
		String filePathLastTraceString = filePathLastTrace.toString();
		...
		for (Future<MatchInstancesGraphRes> f : result) {
			...
			try {
				...
				System.out.println("starting with trace: " +trace.getAttributes().get("concept:name")+ "\n" +
									"the trace is:"+ trace.size() +" long \n"+
						r.queuedStates +" states where queued \n"
						+ r.states +" states");
				
				
				try (BufferedWriter writer = new BufferedWriter(new FileWriter(filePathLastTraceString))) {
					  writer.write(trace.getAttributes().get("concept:name").toString());
			          writer.newLine();
		        } catch (IOException e) {
		            e.printStackTrace();
		        }
				...
				String row = trace.getAttributes().get("concept:name")+","+  trace.size() +"," +r.queuedStates+"," +r.states;
				int states = addReplayResults(delegate, trace, r, doneMap, log, col, r.trace, null,
						new LinkedList<Object>(), new LinkedList<StepTypes>(), filePathString, row);
		...
		System.out.print("Finished creating result");
