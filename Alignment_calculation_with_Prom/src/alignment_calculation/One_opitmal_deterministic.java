...
public class One_opitmal_deterministic {
	...
	XLog log = Prom_without_Prom_Helper.readSingleLog(pathToEventlog.resolve(nameOfEventLog).toFile());
		...
		Path pathToModel = pathToEventlog;
		...
		while (iterator.hasNext()) {
			...
			Path pathToResults = pathToEventlog.resolve("PROM_"+algorithmName + "_runs");
			Files.createDirectories(pathToResults);
			for (int i = 0; i < 10; i++) {
			
			
			String fileName = "resultList_PROM_" + algorithmName+"_run_"+ i+".json";
			...
			ReplayerResultList Replayer =  new ReplayerResultList(parameters, (Petrinet) net, Prom_without_Prom_Helper.getInitialMarking(net), Prom_without_Prom_Helper.getFinalMarking(net), classes, mapping, true);
				...
				for(Future<TraceReplayTask> trace: result) {
					...
					else if (trace.get().getResult().name()=="DUPLICATE") {
				...
				objectMapper.writeValue(pathToResults.resolve(fileName).toFile(), alignmentResults);
